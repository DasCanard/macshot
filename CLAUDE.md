...
- **Offline bitrate targets differ from capture.** Medium/Low MP4 exports use `VideoExportEncodingPlan`, with source metadata loaded during preparation. Do not apply live-recording bitrate minimums or estimate size by multiplying input bytes by a quality percentage. Estimates use the same target settings and edited duration as export, including audio. High/GIF have no comparable target estimate. Keep file/track metadata reads out of editor drawing.
